        ...
        foreach($notes as $note) {
            ...
            echo "
                    <div class='d-flex text-body-secondary pt-3' style='justify-content: space-between;'>
                        ...
                        <strong class='d-block text-gray-dark ml-15' style='font-size:18px'>" . $note['subject'] ."</strong>"
                            . $note['title'] .
                        "</p>
                    ...
                    <div class='d-flex text-body-secondary pt-3' style='justify-content: center; align-items:center'>
                        <button class='m-1 btn btn-outline-success' onclick='payOrder(" . $note['id'] . ")'> Pay it </button>
                        <button class='m-1 btn btn-outline-primary'> View more </button>";
                        if(getOrdersByUserId(1, $note['id'])) {
                            echo "<button class='m-1 btn btn-outline-secondary' onclick='downloadPDF(" . $note['title'] . ")'> Download </button>";
                        }
                    echo "</div>";
        }
    ?>
...
<script>
    ...
    function downloadPDF(noteTitle) {
        var ruta = './resources/pdfs/' + noteTitle;
        var nombre_archivo = noteTitle;
        var url_descarga = ruta + '?nombre=' + nombre_archivo;
        window.location.href = url_descarga;
    }
</script>